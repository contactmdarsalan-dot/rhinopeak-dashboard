...
def settings_payload(settings: WorkspaceSettings | None) -> dict[str, Any]:
    ...
    return {
        ...
        "panVatNumber": getattr(settings, "pan_vat_number", ""),
...
def bootstrap_payload(user: UserAccount) -> dict[str, Any]:
    ...
    return {
        ...
        "parties": list_records(workspace, "parties"),
        "partyLedger": list_records(workspace, "party_ledger"),
        "purchases": list_records(workspace, "purchases"),
        "expenses": list_records(workspace, "expenses"),
        "expenseCategories": [item.get("name", "") for item in list_records(workspace, "expense_categories") if item.get("name")],
        "cashBankAccounts": list_records(workspace, "cash_bank_accounts"),
        "moneyMovements": list_records(workspace, "money_movements"),
        "journalEntries": list_records(workspace, "journal_entries"),
        "documents": list_records(workspace, "documents"),
        "reminderTemplates": list_records(workspace, "reminder_templates"),
        "reminderLogs": list_records(workspace, "reminder_logs"),
        "syncOperations": list_records(workspace, "sync_operations"),
        "customers": list_records(workspace, "customers"),
        "suppliers": list_records(workspace, "suppliers"),
        "creditLedger": list_records(workspace, "credit_ledger"),
        "inventory": list_records(workspace, "inventory"),
        "inventoryCategories": [item.get("name", "") for item in list_records(workspace, "inventory_categories") if item.get("name")],
