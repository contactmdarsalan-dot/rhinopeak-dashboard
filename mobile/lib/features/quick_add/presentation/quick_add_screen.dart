    ...
    return RpPage(
      ...
      child: Column(
        ...
        children: [
          ...
          if (_mode == 0) const _BillScanForm(),
          if (_mode == 1) const _SaleForm(),
          if (_mode == 2) const _ExpenseForm(),
          if (_mode == 3) const _StockMovementForm(),
        ],
      ),
    );
  }
...
class _BillScanForm extends ConsumerStatefulWidget {
  const _BillScanForm();

  @override
  ConsumerState<_BillScanForm> createState() => _BillScanFormState();
}

class _BillScanFormState extends ConsumerState<_BillScanForm>
...
Payment Mode:                Bank''',
  ...
  Future<void> _runScanPipeline(String text, String fileName, String sourceType) async {
    ...
    try {
      ...
      // Step 3: LLM Structuring (Custom PyTorch GPT)
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: isDone
                  ...
                  : (isActive ? theme.colorScheme.primary.withValues(alpha: 0.1) : Colors.transparent),
      ...
      children: [
        ...
        if (state.loading || _scanStep > 0) ...[
          Container(
            padding: const EdgeInsets.all(16),
            decoration: BoxDecoration(
              color: theme.colorScheme.surfaceContainerHighest.withValues(alpha: isDark ? 0.3 : 0.6),
...
class _EditableBillItemTileState extends ConsumerState<_EditableBillItemTile> {
  ...
  void didUpdateWidget(covariant _EditableBillItemTile oldWidget) {
    ...
    if (_nameController.text != newName && !_nameController.hasFocus) {
      _nameController.text = newName;
    }
    if (double.tryParse(_qtyController.text) != double.tryParse(newQtyStr) && !_qtyController.hasFocus) {
      _qtyController.text = newQtyStr;
    }
    if (double.tryParse(_rateController.text) != double.tryParse(newRateStr) && !_rateController.hasFocus) {
