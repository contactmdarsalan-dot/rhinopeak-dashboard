  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12 }}>
        <StatTile label="Current plan" value={plan === 'pro' ? 'Pro' : 'Free'} tone={plan === 'pro' ? 'success' : 'warning'} detail={plan === 'pro' ? `${billingCycle} billing` : `Trial preview ends ${trialEndsAt}`} />
        <StatTile label="Sales usage" value={`${salesUsage}/${plan === 'pro' ? 'unlimited' : planLimits.salesEntries}`} tone={salesUsage > 80 && plan === 'free' ? 'warning' : 'accent'} />
        <StatTile label="Customer profiles" value={`${customerUsage}/${plan === 'pro' ? 'unlimited' : planLimits.customerProfiles}`} />
        <StatTile label="Products" value={`${productUsage}/${plan === 'pro' ? 'unlimited' : planLimits.products}`} />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(280px, 0.8fr) minmax(320px, 1fr)', gap: 16 }}>
        <Panel>
          ...
          <div style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div>
              <p style={{ color: 'var(--text-secondary)', fontSize: 12, marginBottom: 6 }}>Monthly sales entries</p>
            ...
            {['4 KPI dashboard', '30-day revenue chart', 'Top 3 products', 'Low-stock alerts', 'Single owner seat'].map((feature) => (
              <p key={feature} style={{ color: 'var(--text-secondary)', fontSize: 13, display: 'flex', gap: 8, alignItems: 'center' }}>
                <Check size={14} color="var(--success)" /> {feature}
              </p>
            ))}
            {plan === 'pro' ? (
          ...
          <div style={{ padding: 18, display: 'grid', gap: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 14, flexWrap: 'wrap', alignItems: 'center' }}>
              <div>
                ...
                <p style={{ color: 'var(--text-muted)', fontSize: 12 }}>per month, annual discount available</p>
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(210px, 1fr))', gap: 10 }}>
              {proFeatures.map((feature) => (
                <p key={feature} style={{ color: 'var(--text-secondary)', fontSize: 13, display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                  <Check size={14} color="var(--success)" style={{ marginTop: 2, flexShrink: 0 }} /> {feature}
                </p>
              ))}
            </div>
            ...
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)' }}>
                {['Invoice', 'Description', 'Gateway', 'Amount', 'Date', 'Status'].map((header) => (
                  <th key={header} style={{ padding: '11px 14px', color: 'var(--text-muted)', fontSize: 11, fontWeight: 650, textAlign: 'left', textTransform: 'uppercase' }}>{header}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {billingHistory.map((record, index) => (
                <tr key={record.id} style={{ borderBottom: index < billingHistory.length - 1 ? '1px solid var(--border-subtle)' : 'none' }}>
                  <td style={{ padding: '12px 14px', color: 'var(--accent)', fontWeight: 700, fontSize: 12 }}>{record.id}</td>
                  <td style={{ padding: '12px 14px', color: 'var(--text-primary)', fontSize: 13 }}>{record.description}</td>
                  <td style={{ padding: '12px 14px' }}><Badge>{record.gateway}</Badge></td>
                  <td style={{ padding: '12px 14px', color: 'var(--text-primary)', fontWeight: 700, fontSize: 13 }}>{formatCurrency(record.amount)}</td>
                  <td style={{ padding: '12px 14px', color: 'var(--text-muted)', fontSize: 12 }}>{record.date}</td>
                  <td style={{ padding: '12px 14px' }}><Badge tone={record.status === 'Paid' ? 'success' : 'warning'}>{record.status}</Badge></td>
                </tr>
              ))}
            </tbody>
          </table>
