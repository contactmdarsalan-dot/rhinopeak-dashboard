...
export default function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="app-shell" style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg-primary)' }}>
      <Sidebar />
      <div className="app-content-shell" style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, overflow: 'hidden' }}>
        <Topbar />
        {lastNotice && (
          <motion.div 
          ...
          >
            <span>{uiText(settings.language, lastNotice)}</span>
            <button onClick={clearNotice} style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontWeight: 700 }}>{uiText(settings.language, 'Dismiss')}</button>
          </motion.div>
        )}
        <main className="app-main" style={{ flex: 1, padding: 20, overflowY: 'auto', position: 'relative' }}>
